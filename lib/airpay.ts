...
export function prepareAirpayPayment(params: AirpayInitParams): AirpayInitResult {
  ...
  // Match original kit EXACTLY - use raw values, no sanitization for checksum calculation
  ...
  // NOTE: Do NOT uppercase - original kit uses orderid as-is from form field
  ...
  // Date in YYYY-MM-DD format (matching original kit: date.toISOString().split('T')[0])
  ...
  // Generate checksum using the original JavaScript kit formula
  // Original kit alldata: buyerEmail + buyerFirstName + buyerLastName + buyerAddress + buyerCity + buyerState + buyerCountry + amount + orderid + date
  // Original kit checksum: sha256(key + "@" + alldata) where key = sha256(username + "~:~" + password)
  ...
  // Debug logging - print FULL values for verification (matching original kit format)
  console.log("[Airpay] Debug checksum calculation (original kit format):")
  ...
  // Log individual alldata components for verification (original kit order)
  console.log("[Airpay] Alldata components breakdown (original kit order):")
  ...
  console.log("  NOTE: Original kit does NOT include mercid, UID, or subscriptionIndex in alldata")
  ...
  // NOTE: Original kit does NOT include UID or mercid in checksum calculation
  ...
  console.log("  NOTE: UID and mercid are NOT in checksum (original kit format), but are sent in paymentData")
  console.log("[Airpay] Final paymentData object (before cleaning):", JSON.stringify(paymentData, null, 2))

  // CRITICAL: Original JavaScript kit SENDS privatekey in the form
  // Original kit line 43: '<input type="hidden" name="privatekey" id="privatekey" value="'+privateKey+'">'
  // So we MUST include privatekey in the payload (DO NOT remove it)
  ...
  const cleanPaymentData = Object.fromEntries(
    Object.entries(paymentData).filter(([key, v]) => {
      ...
      const requiredFields = [
        ...
        "orderid", "amount", "mercid", "privatekey", "checksum", "kittype"
      ]
  ...
  console.log("[Airpay] Cleaned paymentData (matching original kit format):")
  ...
  console.log("  ✓ privatekey included (required by original kit):", "privatekey" in cleanPaymentData)
